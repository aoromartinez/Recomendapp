...
ipcMain.handle('spotify:request',async(_,endpoint,options={})=>{ const token=await accessToken();for(let attempt=0;attempt<6;attempt++){const res=await fetch(`https://api.spotify.com/v1${endpoint}`,{method:options.method||'GET',headers:{Authorization:`Bearer ${token}`,...(options.body?{'Content-Type':'application/json'}:{})},body:options.body?JSON.stringify(options.body):undefined});if(res.status===429&&attempt<5){const wait=Math.max(1,Number(res.headers.get('retry-after')||2));await new Promise(resolve=>setTimeout(resolve,wait*1000));continue;}const text=await res.text();let data={};try{data=text?JSON.parse(text):{}}catch{}if(!res.ok)throw new Error(data.error?.message||`Spotify respondió ${res.status}.`);return data;}throw new Error('Spotify limitó temporalmente las solicitudes. Intenta de nuevo.'); });
